...
zip_to_zcta_raw = pd.read_excel(
    "ZIPCodetoZCTACrosswalk2021UDS.xlsx", dtype={"ZIP_CODE": object, "ZCTA": object}
)
zip_to_dma_raw = pd.read_csv(
    "zip_to_dma.csv", dtype={"zip_code": object, "dma_code": object}
)
...
zip_to_zcta_dma = zip_to_zcta.merge(
    zip_to_dma, how="left", on="zip_code"
)  # might want full outer?
...
zip_to_zcta_dma["dma_code"] = zip_to_zcta_dma["dma_code"].fillna(
    zip_to_zcta_dma["dma_code_zcta_join"]
)
zip_to_zcta_dma["dma"] = zip_to_zcta_dma["dma"].fillna(zip_to_zcta_dma["dma_zcta_join"])
zip_to_zcta_dma = zip_to_zcta_dma.drop(
    columns=["zip_code_zcta_join", "dma_code_zcta_join", "dma_zcta_join"]
)
