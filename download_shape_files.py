...
state_geom = pygris.states(year=2022, cb = True)
state_geom.to_file("state_geom.shp", index=False)

county_geom = pygris.counties(year=2022, cb = True)
county_geom.to_file("county_geom.shp", index=False)

zcta_geom = pygris.zctas(year=2020, cb = True) # 2020
zcta_geom.to_file("zcta_geom.shp", index=False)

tract_geom = pygris.tracts(year=2022, cb = True)
