...
VARS_PRE2012 = [v for v in VARS if not v.startswith("B23025") and not v.startswith("B15003")]
...
def _process(df):
    ...
    for col in VARS:
        if col in df.columns:
            df[col] = pd.to_numeric(df[col], errors="coerce").replace(-666666666.0, np.nan)
    ...
    df["pct_bachelors_plus"] = (
        df[["_educ_bachelors", "_educ_masters", "_educ_professional", "_educ_doctorate"]].sum(axis=1)
        / df["_educ_total"]
    )
...
ts_state = ts_state.drop(columns=["state"], errors="ignore").rename(columns={"NAME": "state"})
ts_state = ts_state.sort_values(["state", "year"]).reset_index(drop=True)
_log(f"State timeseries complete: {len(ts_state)} rows, {len(ts_state.columns)} columns")
...
_log(f"County timeseries complete: {len(ts_county)} rows, {len(ts_county.columns)} columns")
