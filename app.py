...
block_group_geom_by_city = {
    city: block_group_geom[block_group_geom['GEOID'].str[:5].isin(fips)].set_index('GEOID').to_json()
...
county_metric_cols = sorted(col for col in c_county_state.columns if col not in ['state', 'county', 'state_NAME', 'GEOID', 'NAME'])
zcta_metric_cols = sorted(col for col in c_zcta_dma.columns if col not in ['dma', 'zcta'])
tract_metric_cols = sorted(col for col in c_tract.columns if col not in ['state', 'county', 'state_NAME', 'GEOID', 'NAME', 'tract'])
block_group_metric_cols = sorted(col for col in c_block_group.columns if col not in ['state', 'county', 'state_NAME', 'GEOID', 'NAME', 'tract', 'block group'])
...
TIMESERIES_METRICS = [
    "Pop", "Median Household Income", "Median Home Value", "Median Gross Rent",
    "pct_male", "pct_white_nh", "pct_black", "pct_hispanic", "pct_asian",
    "pct_poverty", "pct_unemployed", "pct_bachelors_plus",
...
_btn_style = {
    'marginRight': '8px', 'padding': '5px 12px', 'cursor': 'pointer',
    'fontFamily': 'Arial', 'fontSize': '13px', 'borderRadius': '4px',
...
CPI = {
    ...
    2021: 270.970, 2022: 292.655,
    2023: 304.702, 2024: 314.175,
}
...
def _apply_cpi(df, year_col='year'):
    ...
    for col in CPI_COLS:
        if col in df.columns:
            df[col] = df[col] * df[year_col].map(lambda y: CPI[2024] / CPI.get(y, CPI[2024]))
    return df
...
def _is_ratio(metric):
    return metric.startswith('pct_') or '_ratio' in metric or 'Median' in metric or metric in ('Median Home Value', 'Median Gross Rent')
...
app.layout = html.Div([
    ...
    dcc.Tabs([
        ...
        dcc.Tab(label='States', style=_tab_style, selected_style=_tab_style, children=[
            html.Div([
                html.Div([
                ...
                html.Div([
                    html.Iframe(id='state_map', width='100%', height='700')
                ], style={'flexGrow': 1, 'padding': '20px'})
            ], style={'display': 'flex', 'alignItems': 'flex-start'})
        ]),

        dcc.Tab(label='DMAs', style=_tab_style, selected_style=_tab_style, children=[
            html.Div([
                html.Div([
                    html.Label("Select Metrics", style={'fontFamily': 'Arial', 'fontWeight': 'bold'}),
                    dcc.Dropdown(
                ...
                html.Div([
                    html.Iframe(id='dma_map', width='100%', height='700')
                ], style={'flexGrow': 1, 'padding': '20px'})
            ], style={'display': 'flex', 'alignItems': 'flex-start'})
        ]),

        dcc.Tab(label='Counties', style=_tab_style, selected_style=_tab_style, children=[
            html.Div([
                html.Div([
                    html.Label("Select Metrics", style={'fontFamily': 'Arial', 'fontWeight': 'bold'}),
                    dcc.Dropdown(
                ...
                html.Div([
                    html.Iframe(id='county_map', width='100%', height='700')
                ], style={'flexGrow': 1, 'padding': '20px'})
            ], style={'display': 'flex', 'alignItems': 'flex-start'})
        ]),

        dcc.Tab(label='ZCTAs', style=_tab_style, selected_style=_tab_style, children=[
            html.Div([
                html.Div([
        ...
        dcc.Tab(label='Tracts', style=_tab_style, selected_style=_tab_style, children=[
            html.Div([
                html.Div([
                    html.Label("Select Metrics", style={'fontWeight': 'bold'}),
                    dcc.Dropdown(
        ...
        dcc.Tab(label='Block Groups', style=_tab_style, selected_style=_tab_style, children=[
            html.Div([
                html.Div([
                    html.Label("Select Metrics", style={'fontWeight': 'bold'}),
                    dcc.Dropdown(
        ...
        dcc.Tab(label='Trends', style=_tab_style, selected_style=_tab_style, children=[
            html.Div([
                html.Span("Suggested: ", style={'fontFamily': 'Arial', 'fontWeight': 'bold', 'marginRight': '8px'}),
                *[
                    html.Button(s['label'], id=f'trends-preset-{i}', n_clicks=0, style=_btn_style)
                    for i, s in enumerate(SUGGESTED_TRENDS)
                ],
            ], style={'padding': '12px 20px', 'borderBottom': '1px solid #eee'}),
            html.Div([
                html.Div([
                    html.Label("Geography Level", style={'fontWeight': 'bold'}),
                    dcc.Dropdown(
                        id='trends-geo-level',
                    ...
                    html.Label("Select Geography", style={'fontWeight': 'bold', 'marginTop': '12px'}),
                    dcc.Dropdown(
                        id='trends-geo',
                    ...
                    html.Label("Metric", style={'fontWeight': 'bold', 'marginTop': '12px'}),
                    dcc.Dropdown(
                        id='trends-metric',
                ...
                html.Div([
                    dcc.Graph(id='trends-chart', style={'height': '700px'})
                ], style={'flexGrow': 1, 'padding': '20px'})
            ], style={'display': 'flex', 'alignItems': 'flex-start'})
        ]),

        dcc.Tab(label='Animated Scatter', style=_tab_style, selected_style=_tab_style, children=[
            html.Div([
                html.Span("Suggested: ", style={'fontFamily': 'Arial', 'fontWeight': 'bold', 'marginRight': '8px'}),
                *[
                    html.Button(s['label'], id=f'anim-preset-{i}', n_clicks=0, style=_btn_style)
            ...
            html.Div([
                html.Div([
                    ...
                    html.Label("X Axis", style={'fontWeight': 'bold', 'marginTop': '12px'}),
                    dcc.Dropdown(
                        id='anim-x',
                        options=TIMESERIES_METRICS,
                        value='pct_poverty',
                    ...
                    html.Label("Y Axis", style={'fontWeight': 'bold', 'marginTop': '12px'}),
                    dcc.Dropdown(
                        id='anim-y',
                        options=TIMESERIES_METRICS,
                        value='Median Household Income',
                    ...
                    html.Label("Color by (optional)", style={'fontWeight': 'bold', 'marginTop': '12px'}),
                    dcc.Dropdown(
                        id='anim-color',
                        options=TIMESERIES_METRICS,
                        value='pct_black',
                    ...
                    html.Label("Size by (optional)", style={'fontWeight': 'bold', 'marginTop': '12px'}),
                    dcc.Dropdown(
                        id='anim-size',
                        options=TIMESERIES_METRICS,
                        value='Pop',
                ...
                html.Div([
                    dcc.Graph(id='anim-scatter-plot', style={'height': '700px'})
                ], style={'flexGrow': 1, 'padding': '20px'})
            ], style={'display': 'flex', 'alignItems': 'flex-start'})
        ]),

        dcc.Tab(label='Scatter', style=_tab_style, selected_style=_tab_style, children=[
            html.Div([
                html.Span("Suggested: ", style={'fontFamily': 'Arial', 'fontWeight': 'bold', 'marginRight': '8px'}),
                *[
                    html.Button(s['label'], id=f'scatter-preset-{i}', n_clicks=0, style=_btn_style)
...
def _build_choropleth_map(geo_json, data_df, id_col, label_key, selected_metrics, name_col=None):
    ...
    for i in selected_metrics:
        ...
        for s in my_chp.geojson.data['features']:
            if s['id'] in id_set:
                label = str(data_df_indexed.loc[s['id'], name_col]) if name_col else s['id']
...
def generate_state_map(selected_metrics, normalize=False):
    ...
    return _build_choropleth_map(state_geom_json, df, 'state', 'State', selected_metrics)
...
def generate_county_map(selected_metrics, normalize=False):
    df = _normalize_df(c_county_state, selected_metrics) if normalize else c_county_state
    return _build_choropleth_map(county_geom_json, df, 'GEOID', 'County', selected_metrics, name_col='NAME')
...
def generate_zcta_map(selected_metrics, selected_dma, pop_min=None, normalize=False):
    ...
    zcta_geom_select = zcta_geom_select[['ZCTA5CE20', 'geometry']].set_index('ZCTA5CE20')
    ...
    return _build_choropleth_map(zcta_json_select, df, 'ZCTA5CE20', 'ZCTA', selected_metrics)


def generate_tract_map(selected_metrics, selected_state, pop_min=None, exclude=None, normalize=False):
    ...
    return _build_choropleth_map(tract_geom_by_state[state_fips], df, 'GEOID', 'Tract', selected_metrics)


def generate_block_group_map(selected_metrics, selected_city, pop_min=None, exclude=None, normalize=False):
    county_fips = _city_fips[selected_city]
    df = c_block_group.loc[c_block_group['GEOID'].str[:5].isin(county_fips)].reset_index(drop=True)
    if pop_min is not None:
        df = df.loc[df[DEFAULT_VAR] >= pop_min]
    if exclude:
        df = df.loc[~df['GEOID'].isin(exclude)]
    ...
    return _build_choropleth_map(block_group_geom_by_city[selected_city], df, 'GEOID', 'Block Group', selected_metrics)
...
@app.callback(
    Output('block-group-exclude', 'options'),
    Input('city-selector', 'value')
)
def update_block_group_exclude_options(selected_city):
    return sorted(c_block_group.loc[
...
def update_scatter(geo, x_metric, y_metric, color_metric, size_metric):
    ...
    fig = px.scatter(
        ...
        color_continuous_scale=_trunc_colorscale(_get_color(color_metric)) if color_metric else _trunc_colorscale('Viridis'),
    )
...
@app.callback(
    ...
    [Input(f'anim-preset-{i}', 'n_clicks') for i in range(len(SUGGESTED_ANIM_SCATTERS))],
    prevent_initial_call=True
)
...
def update_anim_scatter(geo_level, x_metric, y_metric, color_metric, size_metric, inflate):
...
def update_trends_chart(geo_level, geo_names, metric, inflate):
    if not geo_names or not metric:
        return px.line()
    df, name_col = TIMESERIES_GEOS[geo_level]
    plot_df = df[df[name_col].isin(geo_names)][['year', name_col, metric]].dropna(subset=[metric])
    if inflate:
        plot_df = _apply_cpi(plot_df)
    fig = px.line(
        plot_df, x='year', y=metric, color=name_col,
        markers=True,
